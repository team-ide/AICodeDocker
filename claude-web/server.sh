...
function start(){
    ...
    run_command="claude-web --port $port --host $host"
    ...
    if [ "$arg2" = "-D" ]; then
      ...
      if [ $? -eq 0 ]; then
        cat start.pid
        echo "claude-web started successfully!"
      else
        echo "claude-web started failure!"
        exit 1
      fi
      return 0
    else
...
function stop(){
    ...
    ps aux |grep claude-web |grep -v grep |awk '{print "kill -15 " $2}'|sh
    sleep 3
